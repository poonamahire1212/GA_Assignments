window.onload = function(){
    //TODO sort glossary -- write new fucntion
    const sortedGlossary =  glossary.sort(function(a, b) {
    ...
    showResults(sortedGlossary);
    checkboxFilter(glossary);
}
//const blinkingElement = document.getElementById('heading');
//blinkingElement.style.color = "turquoise";
//setInterval(() => blinkingElement.hidden = !blinkingElement.hidden, 1000);

//function for getting all unique tags to display as checkbox
function findUniqueTags(data) {
    //get the array of all tags from glossary
    let arrayOfTags = data.flatMap(element=> element.tags);
    //find unique tags
    let arrayOfTagsWithNoDuplicates = arrayOfTags.reduce(function (previousValue, currentValue) {
        if (previousValue.indexOf(currentValue) === -1) {
          previousValue.push(currentValue)
        }
        return previousValue;
        }, [])
    return arrayOfTagsWithNoDuplicates;
}
//function for making checkbox
function checkboxFilter(data){
    const divForTags = document.getElementById('tags-div');
    let tags = findUniqueTags(glossary);
    tags.forEach((element, index)=>{
        let tagValue = document.createElement('input');
        tagValue.type = "checkbox";
        tagValue.name = "tag";
        tagValue.value = `${element}`;
        tagValue.id = `tag ${index}`;
        ...
        divForTags.appendChild(tagValue);
        divForTags.appendChild(label);
        divForTags.appendChild(br);
    })
}

/////// filter functions
// all filter function will execute when this function gets called
function executeAdvancedSearch() {
    let output = glossary;
    
    output = searchTextInTerm(output)
    
    output = filterByClass(output);
    
    output = filterByTag(output)
    
    showResults(output);    
}
// function to filter data by class
function filterByClass(data) {
    const getSelectedClass = document.getElementById('select-class');
    const classfilterQuery = getSelectedClass.value;
    //if no class is selected show all terms
    if(classfilterQuery === ''){
        return data;
    }else{
        //selecting terms from input data array who has expected class
        let result = data.filter(element=> element.class == classfilterQuery)
        return result;
    }
}

//function to filter data by searching with user input term
function searchTextInTerm(data) {
    const searchField = document.getElementById('searchField');
    if (searchField.value === '' || searchField.value === undefined) {
        return data; // no search query
    } else {
        let searchQuery = searchField.value.toLowerCase();
        //case insensitive search using lowercase data comparison
        let result = data.filter(element => element.term.toLowerCase().includes(searchQuery));
        return result;
    }
}
...
function filterByTag(data) {
    ...
    //if none checkbox is selected then do not perform any operation
    if(checkboxes.length === 0){
        return data;
    }

    //iterate through selected checkboxes and push them in selectedTags array 
    for (var i = 0; i < checkboxes.length; i++) {
        selectedTags.push(checkboxes[i].value)
    }
    //checking selected tags are present in tags from data array
    const result = data.filter(element=>element.tags.some(tag=> selectedTags.indexOf(tag) >= 0))
        return result; 
    }
...
function showResults(data) {
    ...
    //it will clear the div after showing whole result
    results.innerHTML = "";
    ...
    data.forEach(element =>{
        ...
        entry.setAttribute("class", "entry-div-simple");
        //label for term
        let labelTerm = document.createElement('label');
        labelTerm.setAttribute('class', 'label-term');
        labelTerm.innerText = element.term + ': ';

        //label for definition
        let labelDef = document.createElement('label');
        labelDef.setAttribute('class', 'label-def');
        labelDef.innerText = element.definition;

        //adding term and definition into the div we created to show one term and def info
        entry.appendChild(labelTerm);
        entry.appendChild(labelDef);

        // adding the term-def div into the div ehich will show whole result.
